...
Group ownership for all files will be set to the group ownership of the new user's home directory, which is the primary group of the new user unless the user has changed it since directory creation.
...
The fourth octal digit of file modes will remain untouched, but since ownership will be primary UID/primary GID, setuid will be ineffectual and setgid and the sticky bit will likely require group ownership to be reset before they will be useful.
ACL preservation is not tested; Python does not supply a direct method to manipulate file ACLs, and ACL support differs between POSIX, NTFS, and NFSv4 in any event.
