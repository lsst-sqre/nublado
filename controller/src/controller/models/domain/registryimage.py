"""Model for necessary information about registry OCI images."""
...
class RegistryImageVersionClass(Enum):
...
class RegistryImage:
    ...
    version_class: RegistryImageVersionClass | None = None
    ...
    def __str__(self) -> str:
        ...
        dig = f"<{dig}>"
        if (
            self.version_class == RegistryImageVersionClass.SEMVER
            ...
            self.version_class == RegistryImageVersionClass.RSP
    ...
    def _compare(self, other: object) -> int:
        ...
        Beyond that, RegistryImages are only comparable if they are
        the same tag type (RSPTag, Semver, or untagged).  Further, if
        they are RSPTag images, they are only comparable within a
        category (weekly, release, etc).

        Untagged images are only sorted by date, and failing that, digest.
        """
        if not isinstance(other, RegistryImage):
            return NotImplemented
    ...
    def _compare_by_class(self, other_image: Self) -> int:
        ...
        if myclass == otherclass:
            if myclass == RegistryImageVersionClass.RSP:
                return self._compare_rsptags(other_image)
            elif myclass == RegistryImageVersionClass.SEMVER:
    ...
    def to_dict(self) -> JSONRegistryImage:
    ...
    @classmethod
    def from_json(cls, inp: JSONRegistryImage | str) -> Self:
        """Much painful assertion that each field is the right type."""
        if isinstance(inp, str):
            obj = json.loads(inp)
            inp = cast(JSONRegistryImage, obj)
        ...
        if not new_tags:
            new_obj.version_class = RegistryImageVersionClass.UNTAGGED
        elif new_obj.rsp_image_tag is not None:
            new_obj.version_class = RegistryImageVersionClass.RSP
        else:
            new_obj.version_class = RegistryImageVersionClass.SEMVER
        return new_obj


def _gen_rsp_coll() -> dict[str, dict[str, RegistryImage]]:
    retval: dict[str, dict[str, RegistryImage]] = {}
    for typ in RSP_TYPENAMES:
        empty: dict[str, RegistryImage] = {}
        retval[typ] = empty
    return retval


@dataclass
class RegistryImageCollection:
    ...
    untagged: dict[str, RegistryImage] = field(default_factory=dict)
    semver: dict[str, RegistryImage] = field(default_factory=dict)
    rsp: dict[str, dict[str, RegistryImage]] = field(
        default_factory=_gen_rsp_coll
    )
