...
class DockerStorageClient:
    ...
    async def list_tags(self, config: DockerSourceOptions) -> list[str]:
        ...
        total_tags = 0
        _max_tags = 100000  # Raise if necessary someday but surely...
        while total_tags < _max_tags:
            try:
            ...
            if not link_url:
                break
            full_link_url = f"https://{config.registry}{link_url}"
            if url == full_link_url:
                # If it did not change, we are stuck in a loop, so give up.
                break
            total_tags += count
            url = full_link_url
