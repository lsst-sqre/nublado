...
class ImagesFactory:
    ...
    def create_images_manager(self) -> ImagesManager:
        ...
        match self._config.source:
            case DockerSource():
                client = DockerStorageClient(
                    self._config.source.credentials_path,
                    self._http_client,
                    self._logger,
                )
                return DockerImagesManager(client, self._logger)
            case GARSource():
                client = GARStorageClient(self._config.source, self._logger)
                return GARImagesManager(client, self._logger)
