...
class NubladoExecutionError(NubladoError):
    """Error generated by code execution in JupyterLab.
...
class NubladoExecutionTimeoutError(NubladoError):
    """Code execution in JupyterLab timed out.
    ...
    def __init__(
        ...
        *,
        ...
        if started_at:
            failed_at = failed_at or datetime.now(tz=UTC)
            timeout = (failed_at - started_at).total_seconds()
            msg = f"Code execution timed out after {timeout:.02f}s"
        else:
            msg = "Code execution timed out"
        super().__init__(
            msg,
            user,
            started_at=started_at,
    ...
    def __str__(self) -> str:
        if notebook := self.context.notebook:
            if cell := self.context.cell:
                msg = f"cell {cell} of notebook {notebook} timed out"
            else:
                msg = f"cell of notebook {notebook} timed out"
        else:
            msg = "running code timed out"
        return f"{self.user}: {msg}"
    ...
    def to_slack(self) -> SlackMessage:
        message = super().to_slack()
        if self.context.notebook:
            intro = f"Timeout while running `{self.context.notebook}`"
            if self.context.cell:
                intro += f" cell `{self.context.cell}`"
        else:
