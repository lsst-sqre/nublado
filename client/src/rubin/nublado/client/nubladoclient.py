...
class NubladoClient:
    ...
    async def auth_to_lab(self) -> None:
        ...
        If user subdomains are enabled, the first reply may be a redirect to
        the per-user domain of their lab. In this case, record the new base
        URL before continuing, since subsequent API calls will make direct
        ``POST`` and other calls to JupyterLab URLs that cannot be redirected
        and must use the correct base URL.
        ...
        # The first time the Nublado client attempts to contact JupyterLab, we
        # may discover that user subdomains are enabled. In this case, the
        # first reply will be a redirect to the per-user hostname. Detect this
        # case and update self._lab_url so that we go directly to the
        # subdomain in the future.
        #
        # If user subdomains are not in play, the first redirect will be to
        # the JupyterHub OAuth 2 URL on the same hostname. In that case, we
        # should not update self._lab_url.
        ...
        if r.is_redirect:
            ...
            redirect = urlparse(r.headers["Location"])
            if redirect.hostname and current.hostname != redirect.hostname:
                self._lab_url = r.headers["Location"].removesuffix(partial)
                self._logger.debug(
